...
func TaskRequestToEntity(id uuid.UUID, dto *request.TaskRequest, createAt time.Time) *entities.Task {
	return entities.NewTask(id, dto.Title, dto.Description, dto.Status, dto.Assignee, dto.Workspace, createAt)
}

func TaskUpdateToEntity(t *request.TaskRequest) *entities.Task {
